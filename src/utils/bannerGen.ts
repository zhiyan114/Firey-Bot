...
export class BannerPic {
  ...
  public async generate(name: string, imgURL: string): Promise<Buffer> {
    return await startSpan({
      ...
      this.setText(name);
      this.setBorder();
      await this.setProfilePicture(imgURL);
  ...
  private setText(name: string) {
    ...
    ctx.fillText("Welcome to Firey's server! I hope you enjoy your stay here (σ`・∀・)σ", this.canvas.width/2, this.canvas.height/3 + 150, this.canvas.width - 100);
  }
  ...
  /**
    ...
    */
  private async setProfilePicture(url: string) {
    const radius = 60;
    const ctx = this.canvas.getContext("2d");
